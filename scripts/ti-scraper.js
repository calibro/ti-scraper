...
const base_url =
  "https://talosintelligence.com/sb_api/query_lookup?query=%2Fapi%2Fv2%2Frelated_ips%2Fdomain%2F&order=ip%20asc";
const limit = 1000;
    ...
    if (running && data.length > 0) {
      data.forEach(el => {
        ...
        if (dom) {
          ...
          el.third_level_domain =
            dom.subdomain.split(".").slice(-1) + "." + el.second_level_domain;
        }

        //add 'category' based on second level domains
        if (query.includes("fastwebnet.it")) {
        ...
        } else if (query.includes("telecomitalia.it")) {
          if (
            (el.hostname && el.hostname.includes("mail.mp.tim.it")) ||
            (el.hostname && el.hostname.includes("pc.tim.it"))
          ) {
            el.category = "corporate";
          } else if (
            el.hostname &&
            el.hostname.includes("retail.telecomitalia.it")
          ) {
            el.category = "consumer";
          } else {
            el.category = "enterprise";
          }
        } else if (query.includes("swisscom.ch")) {
          if (
            (el.hostname &&
              el.hostname.includes("cgnat.res.cust.swisscom.ch")) ||
            (el.hostname &&
              el.hostname.includes("dynamic.cgnat.res.cust.swisscom.ch")) ||
            (el.hostname &&
              el.hostname.includes("dynamic.wline.res.cust.swisscom.ch"))
          ) {
            el.category = "consumer";
          } else {
            el.category = "enterprise";
      ...
      let now = new Date();
      fs.writeFile(
        __dirname + "/../data/" + query + "-" + now.toISOString() + ".csv",
        csvData,
        function(err) {
          if (err) {
